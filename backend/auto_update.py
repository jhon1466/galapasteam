"""Auto-update utilities for the galapasteam backend."""
...
def apply_pending_update_if_any() -> str:
    ...
    try:
        ...
        if new_version:
            return f"galapasteam updated to {new_version}. Please restart Steam."
        return "galapasteam update applied. Please restart Steam."
...
def _fetch_github_latest(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    headers = {
        ...
        "User-Agent": "galapasteam-Updater",
    }
    if token:
    ...
    try:
        ...
        try:
            proxy_url = "https://galapasteam.vercel.app/api/github-latest"
    ...
    if not zip_url and tag_name:
        zip_url = f"https://galapasteam.vercel.app/api/get-plugin/{tag_name}"
...
def check_for_update_once() -> str:
    ...
    try:
        ...
        return f"galapasteam updated to {latest_version}. Please restart Steam."
...
def _check_and_donate_keys() -> None:
    ...
    try:
        ...
        if not steam_path:
            logger.warn("galapasteam: Cannot donate keys - Steam path not found")
            return
        ...
        if pairs:
            ...
            logger.log("galapasteam: No valid keys found to donate")
            
    except Exception as exc:
        logger.warn(f"galapasteam: Donate keys check failed: {exc}")
...
def restart_steam_internal() -> bool:
    ...
    if not os.path.exists(script_path):
        logger.error(f"galapasteam: restart script not found: {script_path}")
        return False
    try:
        CREATE_NO_WINDOW = 0x08000000
        subprocess.Popen(["cmd", "/C", script_path], creationflags=CREATE_NO_WINDOW)
        logger.log("galapasteam: Restart script launched (hidden)")
        return True
    except Exception as exc:
        logger.error(f"galapasteam: Failed to launch restart script: {exc}")
        return False
...
def check_for_updates_now() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warn(f"galapasteam: CheckForUpdatesNow failed: {exc}")
