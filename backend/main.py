...
def _copy_webkit_files() -> None:
    ...
    logger.log(f"Copying galapasteam web UI from {js_src} to {js_dst}")
    try:
        shutil.copy(js_src, js_dst)
    except Exception as exc:
        logger.error(f"Failed to copy galapasteam web UI: {exc}")
    ...
    if os.path.exists(icon_src):
        try:
            ...
            logger.log(f"Copied galapasteam icon to {icon_dst}")
        except Exception as exc:
            logger.error(f"Failed to copy galapasteam icon: {exc}")
    else:
        logger.warn(f"galapasteam icon not found at {icon_src}")
...
def _inject_webkit_files() -> None:
    ...
    logger.log(f"galapasteam injected web UI: {js_path}")
...
def HasgalapasteamForApp(appid: int, contentScriptQuery: str = "") -> str:
    return has_galapasteam_for_app(appid)


def StartAddViagalapasteam(appid: int, contentScriptQuery: str = "") -> str:
    return start_add_via_galapasteam(appid)


def GetAddViagalapasteamStatus(appid: int, contentScriptQuery: str = "") -> str:
...
def CancelAddViagalapasteam(appid: int, contentScriptQuery: str = "") -> str:
    return cancel_add_via_galapasteam(appid)
...
def DeletegalapasteamForApp(appid: int, contentScriptQuery: str = "") -> str:
    return delete_galapasteam_for_app(appid)
...
def OpenExternalUrl(url: str, contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"galapasteam: OpenExternalUrl failed: {exc}")
...
def GetSettingsConfig(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"galapasteam: GetSettingsConfig failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
    ...
    try:
        ...
        if os.path.exists(themes_path):
            try:
                ...
                logger.warn(f"galapasteam: Failed to read themes.json: {exc}")
                return json.dumps({"success": False, "error": "Failed to read themes.json"})
        else:
            return json.dumps({"success": True, "themes": []})
    except Exception as exc:
        logger.warn(f"galapasteam: GetThemes failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def ApplySettingsChanges(
    ...
    try:
        ...
        try:
            logger.log(
                "galapasteam: ApplySettingsChanges raw argument "
                f"type={type(changes)} value={changes!r}"
            )
            logger.log(f"galapasteam: ApplySettingsChanges kwargs: {kwargs}")
        except Exception:
            pass
        ...
        if isinstance(changes, str) and changes:
            try:
                ...
                logger.warn("galapasteam: Failed to parse changes string payload")
                ...
                if isinstance(payload, dict) and "changes" in payload:
                    ...
                    try:
                        ...
                        logger.warn("galapasteam: Failed to parse changesJson string inside payload")
                        return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if "changesJson" in changes and isinstance(changes["changesJson"], str):
                try:
                    ...
                    logger.warn("galapasteam: Failed to parse changesJson payload from dict")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            ...
            if isinstance(changes_json, dict):
                ...
                try:
                    ...
                    logger.warn("galapasteam: Failed to parse changesJson payload")
                    return json.dumps({"success": False, "error": "Invalid JSON payload"})
            else:
                payload = changes
        ...
        if payload is None:
            ...
            logger.warn(f"galapasteam: Parsed payload is not a dict: {payload!r}")
            return json.dumps({"success": False, "error": "Invalid payload format"})

        try:
            logger.log(f"galapasteam: ApplySettingsChanges received payload: {payload}")
        except Exception:
            pass

        result = apply_settings_changes(payload)
        try:
            logger.log(f"galapasteam: ApplySettingsChanges result: {result}")
        except Exception:
            pass
        response = json.dumps(result)
        try:
            logger.log(f"galapasteam: ApplySettingsChanges response json: {response}")
        except Exception:
            pass
        return response
    except Exception as exc:
        logger.warn(f"galapasteam: ApplySettingsChanges failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableLocales(contentScriptQuery: str = "") -> str:
    try:
        ...
        logger.warn(f"galapasteam: GetAvailableLocales failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetTranslations(contentScriptQuery: str = "", language: str = "", **kwargs: Any) -> str:
    try:
        ...
        logger.warn(f"galapasteam: GetTranslations failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})
...
def GetAvailableThemes(contentScriptQuery: str = "") -> str:
    ...
    try:
        ...
        # Sort themes, but put 'midnight' first
        themes.sort(key=lambda x: (x["value"] != "midnight", x["label"]))
        return json.dumps({"success": True, "themes": themes})
    except Exception as exc:
        logger.warn(f"galapasteam: GetAvailableThemes failed: {exc}")
...
class Plugin:
    ...
    def _load(self):
        logger.log(f"bootstrapping galapasteam plugin, millennium {Millennium.version()}")

        try:
            detect_steam_install_path()
        except Exception as exc:
            logger.warn(f"galapasteam: steam path detection failed: {exc}")
        ...
        try:
            ...
            logger.warn(f"galapasteam: settings initialization failed: {exc}")
