"""Handling of galapasteam add/download flows and related utilities."""
...
def _fetch_app_name(appid: int) -> str:
    ...
    client = ensure_http_client("galapasteam: _fetch_app_name")
    try:
        url = f"https://store.steampowered.com/api/appdetails?appids={appid}"
        logger.log(f"galapasteam: Fetching app name for {appid} from Steam API")
        resp = client.get(url, follow_redirects=True, timeout=10)
        logger.log(f"galapasteam: Steam API response for {appid}: status={resp.status_code}")
        ...
        logger.warn(f"galapasteam: _fetch_app_name failed for {appid}: {exc}")
...
def _append_loaded_app(appid: int, name: str) -> None:
    try:
        ...
        logger.warn(f"galapasteam: _append_loaded_app failed for {appid}: {exc}")
...
def _remove_loaded_app(appid: int) -> None:
    try:
        ...
        logger.warn(f"galapasteam: _remove_loaded_app failed for {appid}: {exc}")
...
def _log_appid_event(action: str, appid: int, name: str) -> None:
    try:
        ...
        logger.warn(f"galapasteam: _log_appid_event failed: {exc}")
...
def _preload_app_names_cache() -> None:
    ...
    try:
        ...
        logger.warn(f"galapasteam: _preload_app_names_cache from logs failed: {exc}")
    ...
    try:
        ...
        logger.warn(f"galapasteam: _preload_app_names_cache from loaded_apps failed: {exc}")
    ...
    try:
        ...
        logger.warn(f"galapasteam: _preload_app_names_cache from applist failed: {exc}")
...
def _load_applist_into_memory() -> None:
    ...
    with APPLIST_LOCK:
        ...
        if not os.path.exists(file_path):
            logger.log("galapasteam: Applist file not found, skipping load")
            APPLIST_LOADED = True  # Mark as loaded to avoid repeated checks
            return
        
        try:
            logger.log("galapasteam: Loading applist into memory...")
                ...
                for entry in data:
                    if isinstance(entry, dict):
                        ...
                        if appid and name and isinstance(name, str) and name.strip():
                            ...
                            count += 1
                logger.log(f"galapasteam: Loaded {count} app names from applist into memory")
            else:
                logger.warn("galapasteam: Applist file has invalid format (expected array)")
            
            APPLIST_LOADED = True
        except Exception as exc:
            logger.warn(f"galapasteam: Failed to load applist into memory: {exc}")
...
def _ensure_applist_file() -> None:
    ...
    if os.path.exists(file_path):
        logger.log("galapasteam: Applist file already exists, skipping download")
        return
    
    logger.log("galapasteam: Applist file not found, downloading...")
    client = ensure_http_client("galapasteam: DownloadApplist")
    
    try:
        logger.log(f"galapasteam: Downloading applist from {APPLIST_URL}")
        resp = client.get(APPLIST_URL, follow_redirects=True, timeout=APPLIST_DOWNLOAD_TIMEOUT)
        logger.log(f"galapasteam: Applist download response: status={resp.status_code}")
        resp.raise_for_status()
        ...
        try:
            ...
            if not isinstance(data, list):
                logger.warn("galapasteam: Downloaded applist has invalid format (expected array)")
                return
        except json.JSONDecodeError as exc:
            logger.warn(f"galapasteam: Downloaded applist is not valid JSON: {exc}")
            return
        ...
        logger.log(f"galapasteam: Successfully downloaded and saved applist file ({len(data)} entries)")
    except Exception as exc:
        logger.warn(f"galapasteam: Failed to download applist file: {exc}")
...
def init_applist() -> None:
    ...
    try:
        ...
        logger.warn(f"galapasteam: Applist initialization failed: {exc}")
...
def _load_games_db_into_memory() -> None:
    ...
    with GAMES_DB_LOCK:
        ...
        if not os.path.exists(file_path):
            logger.log("galapasteam: Games DB file not found, skipping load")
            GAMES_DB_LOADED = True
            return
        
        try:
            logger.log("galapasteam: Loading Games DB into memory...")
            with open(file_path, "r", encoding="utf-8") as handle:
                GAMES_DB_DATA = json.load(handle)
            
            logger.log(f"galapasteam: Loaded Games DB ({len(GAMES_DB_DATA)} entries)")
            GAMES_DB_LOADED = True
        except Exception as exc:
            logger.warn(f"galapasteam: Failed to load Games DB: {exc}")
            GAMES_DB_LOADED = True
...
def _ensure_games_db_file() -> None:
    ...
    if os.path.exists(file_path) and not _is_games_db_cache_stale():
        logger.log("galapasteam: Games DB cache is fresh, skipping download")
        return

    logger.log("galapasteam: Downloading Games DB (cache missing or stale)...")
    client = ensure_http_client("galapasteam: DownloadGamesDB")
    
    try:
        logger.log(f"galapasteam: Downloading Games DB from {GAMES_DB_URL}")
        resp = client.get(GAMES_DB_URL, follow_redirects=True, timeout=60)
        logger.log(f"galapasteam: Games DB download response: status={resp.status_code}")
        resp.raise_for_status()
        ...
        logger.log(f"galapasteam: Successfully downloaded Games DB")
    except Exception as exc:
        logger.warn(f"galapasteam: Failed to download Games DB: {exc}")
...
def init_games_db() -> None:
    ...
    try:
        ...
        logger.warn(f"galapasteam: Games DB initialization failed: {exc}")
...
def _process_and_install_lua(appid: int, zip_path: str) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as archive:
            ...
            for name in names:
                try:
                    ...
                    if name.lower().endswith(".manifest"):
                        ...
                        logger.log(f"galapasteam: Extracted manifest -> {out_path}")
                except Exception as manifest_exc:
                    logger.warn(f"galapasteam: Failed to extract manifest {name}: {manifest_exc}")
        except Exception as depot_exc:
            logger.warn(f"galapasteam: depotcache extraction failed: {depot_exc}")
        ...
        logger.log(f"galapasteam: Installed lua -> {dest_file}")
...
def _download_zip_for_app(appid: int):
    client = ensure_http_client("galapasteam: download")
    apis = load_api_manifest()
    if not apis:
        logger.warn("galapasteam: No enabled APIs in manifest")
    ...
    for api in apis:
        ...
        if "<moapikey>" in template:
            if not morrenus_api_key:
                ...
                logger.log(f"galapasteam: Skipping API '{name}' - Morrenus API key not configured")
                continue
        ...
        logger.log(f"galapasteam: Trying API '{name}'")
        try:
            headers = {"User-Agent": USER_AGENT}
            if _is_download_cancelled(appid):
                logger.log(f"galapasteam: Download cancelled before contacting API '{name}'")
                return
            with client.stream("GET", url, headers=headers, follow_redirects=True) as resp:
                code = resp.status_code
                logger.log(f"galapasteam: API '{name}' status={code}")
                ...
                with open(dest_path, "wb") as output:
                    for chunk in resp.iter_bytes():
                        if not chunk:
                            continue
                        if _is_download_cancelled(appid):
                            logger.log(f"galapasteam: Download cancelled mid-stream for appid={appid}")
                            raise RuntimeError("cancelled")
                        output.write(chunk)
                        state = _get_download_state(appid)
                        ...
                        if _is_download_cancelled(appid):
                            logger.log(f"galapasteam: Download cancelled after writing chunk for appid={appid}")
                            raise RuntimeError("cancelled")
                logger.log(f"galapasteam: Download complete -> {dest_path}")

                if _is_download_cancelled(appid):
                    logger.log(f"galapasteam: Download marked cancelled after completion for appid={appid}")
                    raise RuntimeError("cancelled")
                ...
                try:
                    with open(dest_path, "rb") as fh:
                        ...
                        if magic not in (b"PK\x03\x04", b"PK\x05\x06", b"PK\x07\x08"):
                            ...
                            logger.warn(
                                f"galapasteam: API '{name}' returned non-zip file (magic={magic.hex()}, size={file_size}, preview={content_preview[:50]})"
                            )
                            try:
                                os.remove(dest_path)
                            except Exception:
                    ...
                    logger.warn("galapasteam: Downloaded file not found after download")
                    continue
                except Exception as validation_exc:
                    logger.warn(f"galapasteam: File validation failed for API '{name}': {validation_exc}")
                    try:
                        os.remove(dest_path)
                    except Exception:
                        pass
                ...
                try:
                    if _is_download_cancelled(appid):
                        logger.log(f"galapasteam: Processing aborted due to cancellation for appid={appid}")
                        raise RuntimeError("cancelled")
                    _set_download_state(appid, {"status": "processing"})
                    _process_and_install_lua(appid, dest_path)
                    if _is_download_cancelled(appid):
                        logger.log(f"galapasteam: Installation complete but marked cancelled for appid={appid}")
                        raise RuntimeError("cancelled")
                    try:
                    ...
                    if isinstance(install_exc, RuntimeError) and str(install_exc) == "cancelled":
                        try:
                            ...
                            pass
                        logger.log(f"galapasteam: Cancelled download cleanup complete for appid={appid}")
                        return
                    logger.warn(f"galapasteam: Processing failed -> {install_exc}")
                    _set_download_state(
            ...
            if str(cancel_exc) == "cancelled":
                try:
                    ...
                    pass
                logger.log(f"galapasteam: Download cancelled and cleaned up for appid={appid}")
                return
            logger.warn(f"galapasteam: Runtime error during download for appid={appid}: {cancel_exc}")
            _set_download_state(appid, {"status": "failed", "error": str(cancel_exc)})
            return
        except Exception as err:
            logger.warn(f"galapasteam: API '{name}' failed with error: {err}")
...
def start_add_via_galapasteam(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})

    logger.log(f"galapasteam: StartAddViagalapasteam appid={appid}")
...
def delete_galapasteam_for_app(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
    ...
    for path in paths:
        try:
            ...
            logger.warn(f"galapasteam: Failed to delete {path}: {exc}")
    try:
...
def get_icon_data_url() -> str:
    try:
        ...
        logger.warn(f"galapasteam: GetIconDataUrl failed: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


def has_galapasteam_for_app(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
...
def cancel_add_via_galapasteam(appid: int) -> str:
    try:
        appid = int(appid)
    except Exception:
        return json.dumps({"success": False, "error": "Invalid appid"})
    ...
    logger.log(f"galapasteam: Cancellation requested for appid={appid}")
    return json.dumps({"success": True})
...
def get_installed_lua_scripts() -> str:
    ...
    try:
            ...
            for filename in os.listdir(target_dir):
                ...
                if filename.endswith(".lua") or filename.endswith(".lua.disabled"):
                    try:
                        ...
                        logger.warn(f"galapasteam: Failed to process Lua file {filename}: {exc}")
                        continue

        except Exception as exc:
            logger.warn(f"galapasteam: Failed to scan stplug-in directory: {exc}")
        ...
        logger.warn(f"galapasteam: Failed to get installed Lua scripts: {exc}")
        return json.dumps({"success": False, "error": str(exc)})


__all__ = [
    "cancel_add_via_galapasteam",
    "delete_galapasteam_for_app",
    "dismiss_loaded_apps",
    "fetch_app_name",
    "get_add_status",
    "get_games_database",
    ...
    "has_galapasteam_for_app",
    "init_applist",
    "init_games_db",
    "read_loaded_apps",
    "start_add_via_galapasteam",
]
