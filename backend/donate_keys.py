"""Donate keys functionality for galapasteam backend."""
...
def parse_config_vdf_decryption_keys(steam_path: str) -> List[Tuple[str, str]]:
    ...
    if not os.path.exists(config_path):
        logger.warn(f"galapasteam: config.vdf not found at {config_path}")
        return []
    ...
    try:
        ...
        logger.warn(f"galapasteam: Failed to read config.vdf: {exc}")
        return []
    
    try:
        vdf_data = _parse_vdf_simple(vdf_content)
    except Exception as exc:
        logger.warn(f"galapasteam: Failed to parse config.vdf: {exc}")
        return []
...
def extract_valid_decryption_keys(steam_path: str) -> List[Tuple[str, str]]:
    ...
    if not steam_path or not os.path.exists(steam_path):
        logger.warn(f"galapasteam: Invalid Steam path for donate keys: {steam_path}")
        return []
    
    logger.log("galapasteam: Starting donate keys extraction...")
    ...
    for appid, key in all_pairs:
        if validate_appid_key_pair(appid, key):
            ...
            logger.log(
                f"galapasteam: Invalid appid/key pair skipped: appid={appid!r}, "
                f"key_len={len(key)}, key_valid={bool(re.match(r'^[a-zA-Z0-9]+$', key))}"
            )
    
    logger.log(f"galapasteam: Found {len(valid_pairs)} valid decryption key pairs")
    return valid_pairs
...
def send_donation_keys(pairs: List[Tuple[str, str]]) -> bool:
    ...
    """
    if not pairs:
        logger.log("galapasteam: No keys to donate")
        return False
    ...
    try:
        ...
        logger.log(f"galapasteam: Sending {len(pairs)} appid/key pairs to donation endpoint...")
        ...
        count = len(pairs)
        logger.log(f"galapasteam: Donated AppIDs : {count} - Resp : {status_code}")
        
        if status_code == 200:
            return True
        else:
            logger.log(f"galapasteam: Donation request status : {status_code}")
            return False
            
    except Exception as exc:
        logger.warn(f"galapasteam: Failed to send donation keys: {exc}")
        return False
