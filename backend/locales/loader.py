...
def _ensure_locales_dir() -> None:
    try:
        ...
        logger.warn(f"galapasteam: Failed to ensure locales dir: {exc}")
...
def _read_locale_file(locale: str) -> Tuple[Dict[str, Any], Dict[str, str]]:
    ...
    try:
        ...
        logger.warn(f"galapasteam: Failed to read locale file {path}: {exc}")
        return {}, {}
...
def _write_locale_file(locale: str, meta: Dict[str, Any], strings: Dict[str, str]) -> None:
    ...
    try:
        ...
        logger.warn(f"galapasteam: Failed to write locale file {locale}: {exc}")
...
class LocaleManager:
    ...
    def __init__(self) -> None:
        ...
        self._spanish_strings: Dict[str, str] = {}
        self._spanish_meta: Dict[str, Any] = {}
        self.refresh()
    ...
    def refresh(self) -> None:
        with self._lock:
            ...
            if not strings:
                logger.warn("galapasteam: Default locale en.json is empty or missing.")
                strings = {}
            self._spanish_meta = {**meta, "code": DEFAULT_LOCALE}
            self._spanish_strings = strings.copy()
            self._locales = {}
            ...
            for filename in available_files:
                ...
                if locale_code != DEFAULT_LOCALE:
                    updated = False
                    for key in self._spanish_strings.keys():
                ...
                merged_strings = {}
                for key, spanish_value in self._spanish_strings.items():
                    ...
                    if normalised is not None and locale_code != DEFAULT_LOCALE:
                        ...
                        fallback_value = _normalise_value(spanish_value)
            ...
            if DEFAULT_LOCALE not in self._locales:
                self._locales[DEFAULT_LOCALE] = {
                    "meta": self._spanish_meta,
                    "strings": {
                        key: _normalise_value(value) or PLACEHOLDER_VALUE
                        for key, value in self._spanish_strings.items()
                    },
                    "raw": self._spanish_strings.copy(),
                }
