// galapasteam button injection (standalone plugin)
...
(function () {
    ...
    gamepadCSS.textContent = `
        ...
        .galapasteam-button.active-focus,
        .galapasteam-restart-button.active-focus,
        .galapasteam-icon-button.active-focus {
    ...
    // ALL galapasteam overlays that should block Steam navigation
    const OVERLAY_SELECTORS = [
        '.galapasteam-overlay',
        '.galapasteam-settings-overlay',
        '.galapasteam-fixes-results-overlay',
        '.galapasteam-loading-fixes-overlay',
        '.galapasteam-unfix-overlay',
        '.galapasteam-settings-manager-overlay',
        '.galapasteam-alert-overlay',
        '.galapasteam-confirm-overlay',
        '.galapasteam-loadedapps-overlay'
    ];
    ...
    function isBigPictureMode() {
        if (typeof window.__galapasteam_IS_BIG_PICTURE__ !== 'undefined') {
            return window.__galapasteam_IS_BIG_PICTURE__;
        }
    ...
    function scanFocusableElements() {
        ...
        // Only scan if there's a galapasteam overlay active
        const activeOverlay = document.querySelector(OVERLAY_SELECTOR_STRING);

        if (!activeOverlay) {
            console.log('[Gamepad] No galapasteam overlay active, skipping scan');
    ...
    function pollGamepad() {
        ...
        // Check if there's an active galapasteam overlay
...
(function () {
    ...
    window.__galapasteam_IS_BIG_PICTURE__ = isBigPictureMode();

    // Forward logs to Millennium backend so they appear in the dev console
    function backendLog(message) {
        try {
            if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                Millennium.callServerMethod('galapasteam', 'Logger.log', {
                    message: String(message)
                });
            }
        } catch (err) {
            if (typeof console !== 'undefined' && console.warn) {
                console.warn('[galapasteam] backendLog failed', err);
            }
        }
    }

    backendLog('galapasteam script loaded');
    backendLog('Mode Detection: ' + (window.__galapasteam_IS_BIG_PICTURE__ ? 'BIG PICTURE MODE' : 'NORMAL MODE'));
    // anti-spam state
    const logState = {
        missingOnce: false,
        existsOnce: false
    ...
    function fetchGamesDatabase() {
        ...
        return Millennium.callServerMethod('galapasteam', 'GetGamesDatabase', {
            contentScriptQuery: ''
        })
            .then(function (res) {
            ...
            .catch(function (err) {
                console.warn('[galapasteam] Failed to fetch games database', err);
                return {};
            });
    }
    ...
    function fetchFixes(appid) {
        ...
        return Millennium.callServerMethod('galapasteam', 'CheckForFixes', {
            appid: appid,
            contentScriptQuery: ''
        })
            .then(function (res) {
            ...
            .catch(function (err) {
                console.warn('[galapasteam] Failed to fetch fixes', err);
                return null;
            });
    }
    ...
    function fetchSteamGameName(appid) {
        ...
        return fetch('https://store.steampowered.com/api/appdetails?appids=' + appid + '&filters=basic')
            ...
            .catch(function (err) {
                backendLog('galapasteam: fetchSteamGameName error for ' + appid + ': ' + err);
                return null;
            });
    }
    ...
    function applyTranslationBundle(bundle) {
        ...
        const stored = window.__galapasteamI18n || {};
        if (bundle.language) {
        ...
        window.__galapasteamI18n = stored;
    }
    ...
    const DEFAULT_THEMES = {
        midnight: {
            value: "midnight",
            name: "Midnight Fusion",
            bgPrimary: "#050510",
            bgSecondary: "#0f0f2d",
            bgTertiary: "rgba(5, 5, 16, 0.95)",
            bgHover: "rgba(15, 15, 45, 0.95)",
            bgContainer: "rgba(5, 5, 16, 0.8)",
            bgContainerGradient: "rgba(5, 5, 16, 0.85), #050510",
            accent: "#00f2ff",
            accentLight: "#ff00ff",
            accentDark: "#0088cc",
            border: "rgba(0, 242, 255, 0.3)",
            borderHover: "rgba(0, 242, 255, 0.8)",
            text: "#e0e0ff",
            textSecondary: "#a0a0ff",
            gradient: "linear-gradient(135deg, #ff00ff 0%, #00f2ff 100%)",
            gradientLight: "linear-gradient(135deg, #00f2ff 0%, #ff00ff 100%)",
            shadow: "rgba(0, 242, 255, 0.4)",
            shadowHover: "rgba(255, 0, 255, 0.6)"
        },
        original: {
            name: 'Original',
            bgPrimary: '#1b2838',
    ...
    function loadThemesFromBackend() {
        ...
        return Millennium.callServerMethod('galapasteam', 'GetThemes', {
            contentScriptQuery: ''
        }).then(function (res) {
            try {
                const payload = typeof res === 'string' ? JSON.parse(res) : res;
    ...
    function getCurrentThemeKey() {
        try {
            const settings = window.__galapasteamSettings || {};
            const themeKey = (settings.values || {}).general || {};
            return themeKey.theme || 'midnight';
        } catch (e) {
            return 'midnight';
        }
    }
    ...
    function getCurrentTheme() {
        try {
            ...
            if (!THEMES[themeName]) {
                try {
                    backendLog('galapasteam: Theme ' + themeName + ' not found in THEMES, using midnight. Available: ' + Object.keys(THEMES).join(', '));
                } catch (_) { }
            }
            return theme;
        } catch (e) {
    ...
    function generateThemeStyles(theme) {
        return `
            ...
            .galapasteam-settings-overlay select,
            .galapasteam-settings-manager-overlay select,
            .galapasteam-overlay select,
            .galapasteam-fixes-results-overlay select,
            .galapasteam-loadedapps-overlay select {
            ...
            .galapasteam-settings-overlay select option,
            .galapasteam-settings-manager-overlay select option,
            .galapasteam-overlay select option,
            .galapasteam-fixes-results-overlay select option,
            .galapasteam-loadedapps-overlay select option {
                background-color: ${theme.bgPrimary} !important;
                color: ${theme.text} !important;
            }
            .galapasteam-settings-overlay select option:checked,
            .galapasteam-settings-manager-overlay select option:checked,
            .galapasteam-overlay select option:checked,
            .galapasteam-fixes-results-overlay select option:checked,
            .galapasteam-loadedapps-overlay select option:checked {
                background: ${theme.accent} !important;
                color: ${theme.text} !important;
            }
            .galapasteam-settings-overlay select:hover,
            .galapasteam-settings-manager-overlay select:hover,
            .galapasteam-overlay select:hover,
            .galapasteam-fixes-results-overlay select:hover,
            .galapasteam-loadedapps-overlay select:hover {
                border-color: ${theme.borderHover} !important;
            }
            .galapasteam-settings-overlay select:focus,
            .galapasteam-settings-manager-overlay select:focus,
            .galapasteam-overlay select:focus,
            .galapasteam-fixes-results-overlay select:focus,
            .galapasteam-loadedapps-overlay select:focus {
            ...
            .galapasteam-btn:hover:not([data-disabled="1"]) {
                background: ${theme.bgHover};
                transform: translateY(-2px);
                box-shadow: 0 6px 20px ${theme.shadowHover};
                border-color: ${theme.borderHover};
            }
            .galapasteam-btn.primary {
            ...
            .galapasteam-btn.primary:hover:not([data-disabled="1"]) {
    ...
    function ensureThemeStylesheet(themeKey) {
        const id = 'galapasteam-theme-css';
        ...
        try {
            ...
            backendLog('galapasteam: Theme CSS injection failed: ' + err);
        }
    }

    function ensuregalapasteamStyles() {
        const styleEl = document.getElementById('galapasteam-styles');
        ...
        if (styleEl) {
            ...
            try {
                ...
                style.id = 'galapasteam-styles';
                style.textContent = styles;
                document.head.appendChild(style);
            } catch (err) {
                backendLog('galapasteam: Styles injection failed: ' + err);
            }
        }
    }

    function ensureFontAwesome() {
        if (document.getElementById('galapasteam-fontawesome')) return;
        try {
            const link = document.createElement('link');
            link.id = 'galapasteam-fontawesome';
            link.rel = 'stylesheet';
            ...
            backendLog('galapasteam: Font Awesome injection failed: ' + err);
        }
    }

    function showSettingsPopup() {
        if (document.querySelector('.galapasteam-settings-overlay') || settingsMenuPending) return;
        settingsMenuPending = true;
        ensureTranslationsLoaded(false).catch(function () {
            return null;
        }).finally(function () {
            settingsMenuPending = false;
            if (document.querySelector('.galapasteam-settings-overlay')) return;

            try {
                const d = document.querySelector('.galapasteam-overlay');
                if (d) d.remove();
            } catch (_) { }
            ensuregalapasteamStyles();
            ensureFontAwesome();
            ...
            overlay.className = 'galapasteam-settings-overlay';
            ...
            title.textContent = t('menu.title', 'galapasteam · Menu');
            ...
            if (window.__galapasteam_IS_BIG_PICTURE__) {
            ...
            const removeBtn = createMenuButton('lt-settings-remove-lua', 'menu.removegalapasteam', 'Remove via galapasteam', 'fa-trash-can');
            ...
            if (checkBtn) {
                checkBtn.addEventListener('click', function (e) {
                    ...
                    try {
                        Millennium.callServerMethod('galapasteam', 'CheckForUpdatesNow', {
                            contentScriptQuery: ''
                        }).then(function (res) {
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                const msg = (payload && payload.message) ? String(payload.message) : lt('No updates available.');
                                ShowgalapasteamAlert('galapasteam', msg);
                            } catch (_) { }
                        });
                    } catch (_) { }
                });
            ...
            if (fetchApisBtn) {
                fetchApisBtn.addEventListener('click', function (e) {
                    ...
                    try {
                        Millennium.callServerMethod('galapasteam', 'FetchFreeApisNow', {
                            contentScriptQuery: ''
                        }).then(function (res) {
                            try {
                                const payload = typeof res === 'string' ? JSON.parse(res) : res;
                                ...
                                ShowgalapasteamAlert('galapasteam', text);
                            } catch (_) { }
                        });
                    } catch (_) { }
                });
            ...
            if (fixesMenuBtn) {
                fixesMenuBtn.addEventListener('click', function (e) {
                    ...
                    try {
                        ...
                        const appid = match ? parseInt(match[1], 10) : (window.__galapasteamCurrentAppId || NaN);
                        if (isNaN(appid)) {
                            try {
                                overlay.remove();
                            } catch (_) { }
                            const errText = t('menu.error.noAppId', 'Could not determine game AppID');
                            ShowgalapasteamAlert('galapasteam', errText);
                            return;
                        }

                        Millennium.callServerMethod('galapasteam', 'GetGameInstallPath', {
                            appid,
                            contentScriptQuery: ''
                            ...
                            try {
                                ...
                                if (pathPayload && pathPayload.success && pathPayload.installPath) {
                                    ...
                                    window.__galapasteamGameInstallPath = pathPayload.installPath;
                                }
                                window.__galapasteamGameIsInstalled = isGameInstalled;
                                try {
                                    overlay.remove();
                                } catch (_) { }
                                showFixesLoadingPopupAndCheck(appid);
                            } catch (err) {
                                backendLog('galapasteam: GetGameInstallPath error: ' + err);
                                try {
                                    overlay.remove();
                                } catch (_) { }
                            }
                            ...
                            ShowgalapasteamAlert('galapasteam', errorText);
                        });
                    } catch (err) {
                        backendLog('galapasteam: Fixes Menu button error: ' + err);
                    }
                });
            }
            ...
            try {
                ...
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    Millennium.callServerMethod('galapasteam', 'HasgalapasteamForApp', {
                        appid,
                        contentScriptQuery: ''
                    }).then(function (res) {
                        try {
                            ...
                            if (exists) {
                                const doDelete = function () {
                                    try {
                                        Millennium.callServerMethod('galapasteam', 'DeletegalapasteamForApp', {
                                            appid,
                                            contentScriptQuery: ''
                                        }).then(function () {
                                            try {
                                                window.__galapasteamButtonInserted = false;
                                                window.__galapasteamPresenceCheckInFlight = false;
                                                window.__galapasteamPresenceCheckAppId = undefined;
                                                addgalapasteamButton();
                                                const successText = t('menu.remove.success', 'galapasteam removed for this app.');
                                                ShowgalapasteamAlert('galapasteam', successText);
                                            } catch (err) {
                                                backendLog('galapasteam: post-delete cleanup failed: ' + err);
                                            }
                                        }).catch(function (err) {
                                            const failureText = t('menu.remove.failure', 'Failed to remove galapasteam.');
                                            const errMsg = (err && err.message) ? err.message : failureText;
                                            ShowgalapasteamAlert('galapasteam', errMsg);
                                        });
                                    } catch (err) {
                                        backendLog('galapasteam: doDelete failed: ' + err);
                                    }
                                };
                                ...
                                removeBtn.onclick = function (e) {
                                    ...
                                    const confirmMessage = t('menu.remove.confirm', 'Remove via galapasteam for this game?');
                                    showgalapasteamConfirm('galapasteam', confirmMessage, function () {
                                        doDelete();
                                    }, function () {
                                        try {
                                            showSettingsPopup();
    ...
    function ensureTranslationsLoaded(forceRefresh, preferredLanguage) {
        try {
            if (!forceRefresh && window.__galapasteamI18n && window.__galapasteamI18n.ready) {
                return Promise.resolve(window.__galapasteamI18n);
            }
            if (typeof Millennium === 'undefined' || typeof Millennium.callServerMethod !== 'function') {
                window.__galapasteamI18n = window.__galapasteamI18n || {
                    language: 'en',
                    locales: [],
                    strings: {},
                    ready: false
            ...
            const settingsVals = ((window.__galapasteamSettings || {}).values || {}).general || {};
            ...
            if (!targetLanguage) {
                ...
                targetLanguage = useSteamLang ? steamLang : ((window.__galapasteamI18n && window.__galapasteamI18n.language) || 'en');
            }
            return Millennium.callServerMethod('galapasteam', 'GetTranslations', {
                ...
                return window.__galapasteamI18n;
            }).catch(function (err) {
                backendLog('galapasteam: translation load failed: ' + err);
                window.__galapasteamI18n = window.__galapasteamI18n || {
                    language: 'en',
                    locales: [],
                    strings: {},
            ...
            backendLog('galapasteam: ensureTranslationsLoaded error: ' + err);
            window.__galapasteamI18n = window.__galapasteamI18n || {
                language: 'en',
                locales: [],
                strings: {},
    ...
    function translateText(key, fallback) {
        ...
        try {
            const store = window.__galapasteamI18n;
    ...
    function showTestPopup() {
        ...
        // Avoid duplicates
        if (document.querySelector('.galapasteam-overlay')) return;
        // Close settings popup if open so modals don't overlap
        try {
            const s = document.querySelector('.galapasteam-settings-overlay');
            if (s) s.remove();
        } catch (_) { }
        ...
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        ...
        title.className = 'galapasteam-title';
        title.textContent = t('common.appName', 'galapasteam');

        // API list container
        const apiListContainer = document.createElement('div');
        apiListContainer.className = 'galapasteam-api-list';
        ...
        if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
            Millennium.callServerMethod('galapasteam', 'GetApiList', {
                contentScriptQuery: ''
            }).then(function (res) {
                try {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success && payload.apis && Array.isArray(payload.apis)) {
                        ...
                        payload.apis.forEach((api, index) => {
                            ...
                            apiItem.className = `galapasteam-api-item galapasteam-api-${index}`;
        ...
        body.className = 'galapasteam-status';
        ...
        progressWrap.className = 'galapasteam-progress-wrap';
        ...
        progressBar.className = 'galapasteam-progress-bar';
        progressWrap.appendChild(progressBar);

        const progressInfo = document.createElement('div');
        progressInfo.style.cssText = `display:none;margin-top:8px;font-size:12px;color:${colors.textSecondary};`;
        progressInfo.className = 'galapasteam-progress-info';

        const percent = document.createElement('span');
        percent.className = 'galapasteam-percent';
        percent.textContent = '0%';

        const downloadSize = document.createElement('span');
        downloadSize.className = 'galapasteam-download-size';
        ...
        cancelBtn.className = 'galapasteam-btn galapasteam-cancel-btn';
        ...
        hideBtn.className = 'galapasteam-btn galapasteam-hide-btn';
        ...
        function cancelOperation() {
            ...
            try {
                ...
                const appid = match ? parseInt(match[1], 10) : (window.__galapasteamCurrentAppId || NaN);
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    Millennium.callServerMethod('galapasteam', 'CancelAddViagalapasteam', {
                        appid,
                        contentScriptQuery: ''
                    });
                }
            ...
            const status = overlay.querySelector('.galapasteam-status');
            if (status) status.textContent = lt('Cancelled');
            const cancelBtn = overlay.querySelector('.galapasteam-cancel-btn');
            if (cancelBtn) cancelBtn.style.display = 'none';
            const hideBtn = overlay.querySelector('.galapasteam-hide-btn');
            if (hideBtn) hideBtn.innerHTML = `<span>${lt('Close')}</span>`;
            // Hide progress UI
            const wrap = overlay.querySelector('.galapasteam-progress-wrap');
            const progressInfo = overlay.querySelector('.galapasteam-progress-info');
    ...
    function showFixesResultsPopup(data, isGameInstalled) {
        if (document.querySelector('.galapasteam-fixes-results-overlay')) return;
        // Close other popups
        try {
            const d = document.querySelector('.galapasteam-overlay');
            if (d) d.remove();
        ...
        try {
            const f = document.querySelector('.galapasteam-fixes-results-overlay');
            if (f) f.remove();
        } catch (_) { }
        try {
            const l = document.querySelector('.galapasteam-loading-fixes-overlay');
            if (l) l.remove();
        } catch (_) { }
        ...
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-fixes-results-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        ...
        const title = document.createElement('div');
        title.style.cssText = `font-size:24px;color:${colors.text};font-weight:700;text-shadow:0 2px 8px ${colors.shadow};background:${colors.gradientLight};-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;`;
        title.textContent = lt('galapasteam · Fixes Menu');
        ...
        function createFixButton(label, text, icon, isSuccess, onClick) {
            ...
            try {
                const currentThemeKey = (((window.__galapasteamSettings || {}).values || {}).general || {}).theme || 'original';
        ...
        const genericSection = createFixButton(
            ...
            function (e) {
                ...
                if (genericStatus === 200 && isGameInstalled) {
                    const genericUrl = 'https://files.galapasteam.work/GameBypasses/' + data.appid + '.zip';
        ...
        const onlineSection = createFixButton(
            ...
            function (e) {
                ...
                if (onlineStatus === 200 && isGameInstalled) {
                    const onlineUrl = data.onlineFix.url || ('https://files.galapasteam.work/OnlineFix1/' + data.appid + '.zip');
        ...
        const unfixSection = createFixButton(
            ...
            function (e) {
                ...
                if (isGameInstalled) {
                    ...
                    showgalapasteamConfirm('galapasteam', lt('Are you sure you want to un-fix? This will remove fix files and verify game files.'),
                        function () {
        ...
        setTimeout(function () {
            ...
            if (shayenviLink) {
                shayenviLink.addEventListener('click', function (e) {
                    e.preventDefault();
                    try {
                        Millennium.callServerMethod('galapasteam', 'OpenExternalUrl', {
        ...
        gameFolderBtn.className = 'galapasteam-btn';
        ...
        gameFolderBtn.onclick = function (e) {
            e.preventDefault();
            if (window.__galapasteamGameInstallPath) {
                try {
                    Millennium.callServerMethod('galapasteam', 'OpenGameFolder', {
                        path: window.__galapasteamGameInstallPath,
                        contentScriptQuery: ''
                    });
                } catch (err) {
                    backendLog('galapasteam: Failed to open game folder: ' + err);
                }
            }
        };
        ...
        backBtn.className = 'galapasteam-btn';
        ...
        function startUnfix(appid) {
            try {
                Millennium.callServerMethod('galapasteam', 'UnFixGame', {
                    appid: appid,
                    installPath: window.__galapasteamGameInstallPath,
                    contentScriptQuery: ''
                }).then(function (res) {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success) {
                        ...
                        ShowgalapasteamAlert('galapasteam', errorMsg);
                    }
                }).catch(function () {
                ...
                backendLog('galapasteam: Un-Fix start error: ' + err);
            }
        }
    }

    function showFixesLoadingPopupAndCheck(appid) {
        if (document.querySelector('.galapasteam-loading-fixes-overlay')) return;
        try {
            const d = document.querySelector('.galapasteam-overlay');
            if (d) d.remove();
        } catch (_) { }
        ...
        try {
            const f = document.querySelector('.galapasteam-fixes-overlay');
            if (f) f.remove();
        } catch (_) { }
        ...
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-loading-fixes-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        ...
        fetchFixes(appid).then(function (payload) {
            if (payload && payload.success) {
                const isGameInstalled = window.__galapasteamGameIsInstalled === true;
    ...
    function applyFix(appid, downloadUrl, fixType, gameName, resultsOverlay) {
        try {
            ...
            backendLog('galapasteam: Applying fix ' + fixType + ' for appid ' + appid);

            // Start the download and extraction process
            Millennium.callServerMethod('galapasteam', 'ApplyGameFix', {
                appid: appid,
                ...
                try {
                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
                    if (payload && payload.success) {
                        // Show download progress popup similar to Add via GalapaSteam
                    ...
                    backendLog('galapasteam: ApplyGameFix response error: ' + err);
                    const msg = lt('Error applying fix');
                    ShowgalapasteamAlert('galapasteam', msg);
                }
            }).catch(function (err) {
                backendLog('galapasteam: ApplyGameFix error: ' + err);
                const msg = lt('Error applying fix');
                ShowgalapasteamAlert('galapasteam', msg);
            });
        } catch (err) {
            backendLog('galapasteam: applyFix error: ' + err);
        }
    }

    // Show fix download progress popup
    function showFixDownloadProgress(appid, fixType) {
        // Reuse the download popup UI from Add via GalapaSteam
        if (document.querySelector('.galapasteam-overlay')) return;
        ...
        const hideBtn = document.createElement('a');
        hideBtn.href = '#';
        hideBtn.className = 'galapasteam-btn';
        ...
        const cancelBtn = document.createElement('a');
        cancelBtn.href = '#';
        cancelBtn.className = 'galapasteam-btn primary';
        ...
        cancelBtn.onclick = function (e) {
            ...
            Millennium.callServerMethod('galapasteam', 'CancelApplyFix', {
                appid: appid,
                contentScriptQuery: ''
            }).then(function (res) {
                try {
                    ...
                    backendLog('galapasteam: CancelApplyFix response error: ' + err);
                ...
                const msgEl2 = document.getElementById('lt-fix-progress-msg');
                if (msgEl2 && msgEl2.dataset.last) msgEl2.textContent = msgEl2.dataset.last;
                backendLog('galapasteam: CancelApplyFix error: ' + err);
                const msg = lt('Failed to cancel fix download');
                ShowgalapasteamAlert('galapasteam', msg);
            });
        };
    ...
    function replaceFixButtonsWithClose(overlayEl) {
        ...
        closeBtn.href = '#';
        closeBtn.className = 'galapasteam-btn primary';
    ...
    function pollFixProgress(appid, fixType) {
        const poll = function () {
            try {
                const overlayEl = document.querySelector('.galapasteam-overlay');
                if (!overlayEl) return; // Stop if overlay was closed

                Millennium.callServerMethod('galapasteam', 'GetApplyFixStatus', {
                    appid: appid,
                    contentScriptQuery: ''
                }).then(function (res) {
                    try {
                        ...
                        backendLog('galapasteam: GetApplyFixStatus error: ' + err);
                    }
                });
            } catch (err) {
                backendLog('galapasteam: pollFixProgress error: ' + err);
            }
        };
        setTimeout(poll, 500);
    }
    ...
    function showUnfixProgress(appid) {
        ...
        try {
            const old = document.querySelector('.galapasteam-unfix-overlay');
            if (old) old.remove();
        } catch (_) { }
        ...
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-unfix-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
    ...
    function pollUnfixProgress(appid) {
        const poll = function () {
            try {
                const overlayEl = document.querySelector('.galapasteam-unfix-overlay');
                if (!overlayEl) return; // Stop if overlay was closed

                Millennium.callServerMethod('galapasteam', 'GetUnfixStatus', {
                    appid: appid,
                    contentScriptQuery: ''
                }).then(function (res) {
                    try {
                            ...
                            if (state.status === 'removing') {
                                ...
                                setTimeout(function () {
                                    try {
                                        ...
                                        backendLog('galapasteam: Running verify for appid ' + appid);
                                    } catch (_) { }
                                }, 1000);
                        ...
                        backendLog('galapasteam: GetUnfixStatus error: ' + err);
                    }
                });
            } catch (err) {
                backendLog('galapasteam: pollUnfixProgress error: ' + err);
            }
        };
        setTimeout(poll, 500);
    }
    ...
    function fetchSettingsConfig(forceRefresh) {
        try {
            if (!forceRefresh && window.__galapasteamSettings && Array.isArray(window.__galapasteamSettings.schema)) {
                return Promise.resolve(window.__galapasteamSettings);
            }
        } catch (_) { }

        if (typeof Millennium === 'undefined' || typeof Millennium.callServerMethod !== 'function') {
            return Promise.reject(new Error(lt('galapasteam backend unavailable')));
        }

        return Millennium.callServerMethod('galapasteam', 'GetSettingsConfig', {
            contentScriptQuery: ''
        }).then(function (res) {
            const payload = typeof res === 'string' ? JSON.parse(res) : res;
            if (!payload || payload.success !== true) {
            ...
            window.__galapasteamSettings = config;
            return config;
        });
    }
    ...
    function showSettingsManagerPopup(forceRefresh, onBack) {
        if (document.querySelector('.galapasteam-settings-manager-overlay')) return;

        try {
            const mainOverlay = document.querySelector('.galapasteam-settings-overlay');
        ...
        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-settings-manager-overlay';
        ...
        title.textContent = t('settings.title', 'galapasteam · Settings');
        ...
        searchInput.id = 'galapasteam-settings-search';
        ...
        contentWrap.id = 'galapasteam-content-wrap';
        ...
        function applySearchFilter() {
            ...
            const fixesSection = document.getElementById('galapasteam-installed-fixes-section');
            ...
            if (fixesSection && query && fixItems.length > 0 && visibleFixes === 0) {
                if (!fixesEmptySearch) {
                    ...
                    const listContainer = fixesSection.querySelector('#galapasteam-fixes-list');
            ...
            const luaSection = document.getElementById('galapasteam-installed-lua-section');
            ...
            if (luaSection && query && luaItems.length > 0 && visibleLua === 0) {
                if (!luaEmptySearch) {
                    ...
                    emptyEl.style.cssText = `padding:14px;background:${emptyColors.bgTertiary};border:1px solid ${emptyColors.border};border-radius:4px;color:${emptyColors.textSecondary};text-align:center;margin-top:10px;`;
                    emptyEl.textContent = t('settings.search.noResults', 'No matches found');
                    const listContainer = luaSection.querySelector('#galapasteam-lua-list');
                    if (listContainer) listContainer.appendChild(emptyEl);
                }
        ...
        function setStatus(text, color) {
            let statusLine = contentWrap.querySelector('.galapasteam-settings-status');
            if (!statusLine) {
                statusLine = document.createElement('div');
                statusLine.className = 'galapasteam-settings-status';
                ...
                for (let j = 0; j < options.length; j++) {
                    ...
                    if (option.type === 'select') {
                        ...
                        selectEl.addEventListener('change', function () {
                            ...
                            try {
                                backendLog('galapasteam: ' + option.key + ' select changed to ' + selectEl.value);
                            } catch (_) { }

                            // If theme changed, apply it immediately
                            if (group.key === 'general' && option.key === 'theme') {
                                try {
                                    backendLog('galapasteam: Theme change detected, new value: ' + selectEl.value);
                                } catch (_) { }
                                // Update the settings cache so getCurrentTheme() returns the new value
                                if (window.__galapasteamSettings && window.__galapasteamSettings.values) {
                                    if (!window.__galapasteamSettings.values.general) {
                                        window.__galapasteamSettings.values.general = {};
                                    }
                                    window.__galapasteamSettings.values.general.theme = selectEl.value;
                                    try {
                                        backendLog('galapasteam: Updated cache, theme is now: ' + window.__galapasteamSettings.values.general.theme);
                                    } catch (_) { }
                                }
                                ...
                                setTimeout(function () {
                                    ...
                                    const contentWrapEl = overlay && overlay.querySelector('#galapasteam-content-wrap');
                                    if (contentWrapEl) {
        ...
        function renderInstalledFixesSection() {
            ...
            sectionEl.id = 'galapasteam-installed-fixes-section';
            ...
            listContainer.id = 'galapasteam-fixes-list';
        ...
        function loadInstalledFixes(container) {
            ...
            Millennium.callServerMethod('galapasteam', 'GetInstalledFixes', {
                contentScriptQuery: ''
            })
                .then(function (res) {
        ...
        function createFixListItem(fix, container) {
            ...
            deleteBtn.addEventListener('click', function (e) {
                ...
                showgalapasteamConfirm(
                    fix.gameName || 'galapasteam',
        ...
        function renderInstalledLuaSection() {
            const sectionEl = document.createElement('div');
            sectionEl.id = 'galapasteam-installed-lua-section';
            ...
            const listContainer = document.createElement('div');
            listContainer.id = 'galapasteam-lua-list';
            listContainer.style.cssText = 'min-height:50px;';
            sectionEl.appendChild(listContainer);
        ...
        function loadInstalledLuaScripts(container) {
            ...
            Millennium.callServerMethod('galapasteam', 'GetInstalledLuaScripts', {
                contentScriptQuery: ''
            })
                .then(function (res) {
                    const response = typeof res === 'string' ? JSON.parse(res) : res;
                    ...
                    if (hasUnknownGames) {
                        ...
                        infoBanner.innerHTML = '<i class="fa-solid fa-circle-info" style="font-size:16px;"></i><span>' + t('settings.installedLua.unknownInfo', 'Games showing \'Unknown Game\' were installed manually (not via galapasteam).') + '</span>';
        ...
        function createLuaListItem(script, container) {
            ...
            deleteBtn.addEventListener('click', function (e) {
                ...
                showgalapasteamConfirm(
                    script.gameName || 'galapasteam',
                    t('settings.installedLua.deleteConfirm', 'Remove via galapasteam for this game?'),
                    function () {
                        // User confirmed
                        deleteBtn.dataset.busy = '1';
                        deleteBtn.style.opacity = '0.6';
        ...
        saveBtn.addEventListener('click', function (e) {
            ...
            try {
                backendLog('galapasteam: collectChanges payload ' + JSON.stringify(changes));
            } catch (_) { }
            ...
            try {
                backendLog('galapasteam: sending settings payload ' + JSON.stringify(payloadToSend));
            } catch (_) { }
            // Pass flattened keys so Millennium handles the RPC arguments as expected.
            Millennium.callServerMethod('galapasteam', 'ApplySettingsChanges', {
                contentScriptQuery: '',
                ...
                try {
                    if (window.__galapasteamSettings) {
                        window.__galapasteamSettings.values = JSON.parse(JSON.stringify(state.config.values));
                        window.__galapasteamSettings.schemaVersion = state.config.schemaVersion;
                        window.__galapasteamSettings.lastFetched = Date.now();
                        if (response && response.translations && typeof response.translations === 'object') {
                            window.__galapasteamSettings.translations = response.translations;
                        }
                        if (response && response.language) {
                            window.__galapasteamSettings.language = response.language;
                        }
                    }
                } catch (_) { }
                ...
                try {
                    if (window.__galapasteamSettings) {
                        window.__galapasteamSettings.schema = null;
                    }
                } catch (_) { }

                if (response && response.translations && typeof response.translations === 'object') {
                    applyTranslationBundle({
                        language: response.language || (window.__galapasteamI18n && window.__galapasteamI18n.language) || 'en',
                        locales: (window.__galapasteamI18n && window.__galapasteamI18n.locales) || (state.config && state.config.locales) || [],
    ...
    function closeSettingsOverlay() {
        try {
            ...
            var list = document.getElementsByClassName('galapasteam-settings-overlay');
            ...
            var list2 = document.getElementsByClassName('galapasteam-overlay');
    ...
    function showgalapasteamAlert(title, message, onClose) {
        if (document.querySelector('.galapasteam-alert-overlay')) return;

        ensuregalapasteamStyles();
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-alert-overlay';
        ...
        titleEl.textContent = String(title || 'galapasteam');
        ...
        okBtn.href = '#';
        okBtn.className = 'galapasteam-btn primary';
    ...
    function ShowgalapasteamAlert(title, message) {
        try {
            showgalapasteamAlert(title, message);
        } catch (err) {
            backendLog('galapasteam: Alert error, falling back: ' + err);
            try {
                alert(String(title) + '\n\n' + String(message));
            } catch (_) { }
        }
    ...
    function showgalapasteamConfirm(title, message, onConfirm, onCancel) {
        // Always close settings popup first so the confirm is visible on top
        closeSettingsOverlay();

        // Create custom modern confirmation dialog
        if (document.querySelector('.galapasteam-confirm-overlay')) return;

        ensuregalapasteamStyles();
        ensureFontAwesome();
        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-confirm-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(10px);z-index:100001;display:flex;align-items:center;justify-content:center;';
        ...
        const cancelBtn = document.createElement('a');
        cancelBtn.href = '#';
        cancelBtn.className = 'galapasteam-btn';
        cancelBtn.style.flex = '1';
        cancelBtn.innerHTML = `<span>${lt('Cancel')}</span>`;
        cancelBtn.onclick = function (e) {
            e.preventDefault();
        ...
        confirmBtn.href = '#';
        confirmBtn.className = 'galapasteam-btn primary';
    ...
    function showDlcWarning(appid, fullgameAppid, fullgameName) {
        ...
        closeSettingsOverlay();
        if (document.querySelector('.galapasteam-dlc-warning-overlay')) return;

        ensuregalapasteamStyles();
        ensureFontAwesome();

        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-dlc-warning-overlay galapasteam-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(10px);z-index:100001;display:flex;align-items:center;justify-content:center;';
        ...
        goBtn.className = 'galapasteam-btn primary';
    ...
    function showgalapasteamPlayableWarning(message, onProceed, onCancel) {
        // Close settings so modal is visible
        closeSettingsOverlay();
        if (document.querySelector('.galapasteam-playable-warning-overlay')) return;

        ensuregalapasteamStyles();
        ensureFontAwesome();

        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-playable-warning-overlay galapasteam-overlay';
        ...
        proceedBtn.href = '#';
        proceedBtn.className = 'galapasteam-btn primary';
    ...
    function showMillenniumDisclaimerModal() {
        if (document.querySelector('.galapasteam-disclaimer-overlay')) return;

        ensuregalapasteamStyles();
        ensureFontAwesome();

        const overlay = document.createElement('div');
        overlay.className = 'galapasteam-disclaimer-overlay galapasteam-overlay';
        ...
        line1.textContent = t('disclaimer.line1', 'galapasteam is not affiliated in any way with Millennium');
        ...
        line3.textContent = t('disclaimer.line3', 'You will be BANNED from both galapasteam and Millennium servers if you go to their discord asking for help');
        ...
        confirmBtn.onclick = function (e) {
            ...
            if (input.value.trim().toLowerCase() === expectedPhrase) {
                localStorage.setItem('galapasteam millennium disclaimer accepted', '1');
                overlay.remove();
            }
        };
    ...
    function ensureStyles() {
        if (!document.getElementById('galapasteam-spacing-styles')) {
            const style = document.createElement('style');
            style.id = 'galapasteam-spacing-styles';
            style.textContent = `
                .galapasteam-restart-button, .galapasteam-icon-button { margin-left: 6px !important; margin-right: 0 !important; }
                .galapasteam-button { margin-right: 0 !important; position: relative !important; }
                .galapasteam-pills-container {
                ...
                .galapasteam-pill.red { background: rgba(255, 80, 80, 0.15); color: #ff5050; border: 1px solid rgba(255, 80, 80, 0.3); }
                .galapasteam-pill.green { background: rgba(92, 184, 92, 0.15); color: #5cb85c; border: 1px solid rgba(92, 184, 92, 0.3); }
                .galapasteam-pill.yellow { background: rgba(255, 193, 7, 0.15); color: #ffc107; border: 1px solid rgba(255, 193, 7, 0.3); }
                .galapasteam-pill.orange { background: rgba(255, 136, 0, 0.15); color: #ff8800; border: 1px solid rgba(255, 136, 0, 0.3); }
                .galapasteam-pill.gray { background: rgba(150, 150, 150, 0.15); color: #a0a0a0; border: 1px solid rgba(150, 150, 150, 0.3); }
            `;
    ...
    function updateButtonTranslations() {
        try {
            ...
            const restartBtn = document.querySelector('.galapasteam-restart-button');
            if (restartBtn) {
            ...
            // Update Add via GalapaSteam button
            const galapasteamBtn = document.querySelector('.galapasteam-button');
            if (galapasteamBtn) {
                const addViaText = lt('Add via GalapaSteam');
                galapasteamBtn.title = addViaText;
                galapasteamBtn.setAttribute('data-tooltip-text', addViaText);
                const span = galapasteamBtn.querySelector('span');
                if (span) {
                    span.textContent = addViaText;
                }
            }
        } catch (err) {
            backendLog('galapasteam: updateButtonTranslations error: ' + err);
        }
    }

    // Function to add the galapasteam button
    ...
    function addgalapasteamButton() {
        ...
        if (window.__galapasteamLastUrl !== currentUrl) {
            // Page changed - reset button insertion flag and update translations
            window.__galapasteamLastUrl = currentUrl;
            window.__galapasteamButtonInserted = false;
            window.__galapasteamRestartInserted = false;
            window.__galapasteamIconInserted = false;
            window.__galapasteamHeaderInserted = false;
            window.__galapasteamPresenceCheckInFlight = false;
            window.__galapasteamPresenceCheckAppId = undefined;
        ...
        if (!isAppPath) {
            ...
            if (headerContainer && !document.querySelector('.galapasteam-header-button') && !window.__galapasteamHeaderInserted) {
                ensuregalapasteamStyles();
                const headerBtn = document.createElement('a');
                headerBtn.href = '#';
                // Use galapasteam-btn primary class for that premium modal look
                headerBtn.className = 'galapasteam-btn primary galapasteam-header-button Focusable';
                ...
                headerBtn.title = 'galapasteam Settings';

                headerBtn.setAttribute('data-tooltip-text', 'galapasteam Settings');
                ...
                try {
                    Millennium.callServerMethod('galapasteam', 'GetIconDataUrl', {
                        contentScriptQuery: ''
                    }).then(function (res) {
                        try {
                            const payload = typeof res === 'string' ? JSON.parse(res) : res;
                            if (payload && payload.success && payload.dataUrl) {
                                ...
                                img.src = 'galapasteam/galapasteam-icon.png';
                            }
                        } catch (_) {
                            img.src = 'galapasteam/galapasteam-icon.png';
                        }
                ...
                window.__galapasteamHeaderInserted = true;
                backendLog('Inserted store header button (non-app page)');
            }
        }

        // Check if we're in Big Picture mode
        const isBigPicture = window.__galapasteam_IS_BIG_PICTURE__;
        ...
        if (targetContainer) {
            ...
            // Insert a Restart Steam button between Community Hub and our galapasteam button
            try {
                if (!document.querySelector('.galapasteam-restart-button') && !window.__galapasteamRestartInserted) {
                    ensureStyles();
                    ...
                    if (referenceBtn && referenceBtn.className) {
                        restartBtn.className = referenceBtn.className + ' galapasteam-restart-button';
                    } else {
                        restartBtn.className = 'btnv6_blue_hoverfade btn_medium galapasteam-restart-button';
                    }
                    restartBtn.href = '#';
                    const restartText = lt('Restart Steam');
                    restartBtn.title = restartText;
                    ...
                    restartBtn.addEventListener('click', function (e) {
                        ...
                        try {
                            ...
                            closeSettingsOverlay();
                            showgalapasteamConfirm('galapasteam', lt('Restart Steam now?'),
                                function () {
                                    try {
                                        Millennium.callServerMethod('galapasteam', 'RestartSteam', {
                                            contentScriptQuery: ''
                                        });
                                    } catch (_) { }
                                },
                    ...
                    try {
                        if (!document.querySelector('.galapasteam-icon-button') && !window.__galapasteamIconInserted) {
                            // Use same custom button for both modes
                            const iconBtn = document.createElement('a');
                            if (referenceBtn && referenceBtn.className) {
                                iconBtn.className = referenceBtn.className + ' galapasteam-icon-button';
                            } else {
                                iconBtn.className = 'btnv6_blue_hoverfade btn_medium galapasteam-icon-button';
                            }
                            iconBtn.href = '#';
                            iconBtn.title = 'galapasteam Helper';
                            iconBtn.setAttribute('data-tooltip-text', 'galapasteam Helper');
                            ...
                            window.__galapasteamIconInserted = true;
                            backendLog('Inserted Icon button');
                        }
                    } catch (_) { }
                    window.__galapasteamRestartInserted = true;
            ...
            const existingBtn = document.querySelector('.galapasteam-button');
            if (existingBtn) {
                ensureTranslationsLoaded(false).then(function () {
                    updateButtonTranslations();
                });
            ...
            if (!existingBtn && !window.__galapasteamButtonInserted) {

                // Create the galapasteam button modeled after existing SteamDB/PCGW buttons
                // In Big Picture mode, use queue button as reference; otherwise use first link in container
                ...
                // Use same custom button for both modes
                const galapasteamButton = document.createElement('a');
                galapasteamButton.href = '#';
                // Copy classes from an existing button to match look-and-feel, but set our own label
                if (referenceBtn && referenceBtn.className) {
                    galapasteamButton.className = referenceBtn.className + ' galapasteam-button';
                } else {
                    galapasteamButton.className = 'btnv6_blue_hoverfade btn_medium galapasteam-button';
                }
                const span = document.createElement('span');
                const addViaText = lt('Add via GalapaSteam');
                span.textContent = addViaText;
                galapasteamButton.appendChild(span);
                // Tooltip/title
                galapasteamButton.title = addViaText;
                galapasteamButton.setAttribute('data-tooltip-text', addViaText);

                // Normalize margins to match native buttons
                try {
                    if (referenceBtn) {
                        const cs = window.getComputedStyle(referenceBtn);
                        galapasteamButton.style.marginLeft = cs.marginLeft;
                        galapasteamButton.style.marginRight = cs.marginRight;
                    }
                } catch (_) { }

                // Local click handler suppressed; delegated handler manages actions
                galapasteamButton.addEventListener('click', function (e) {
                    e.preventDefault();
                    backendLog('galapasteam button clicked (delegated handler will process)');
                });

                // Before inserting, ask backend if galapasteam already exists for this appid
                try {
                    const match = window.location.href.match(/https:\/\/store\.steampowered\.com\/app\/(\d+)/) || window.location.href.match(/https:\/\/steamcommunity\.com\/app\/(\d+)/);
                    const appid = match ? parseInt(match[1], 10) : NaN;
                    if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                        // prevent multiple concurrent checks
                        if (window.__galapasteamPresenceCheckInFlight && window.__galapasteamPresenceCheckAppId === appid) {
                            return;
                        }
                        window.__galapasteamPresenceCheckInFlight = true;
                        window.__galapasteamPresenceCheckAppId = appid;
                        window.__galapasteamCurrentAppId = appid;
                        Millennium.callServerMethod('galapasteam', 'HasgalapasteamForApp', {
                            appid,
                            contentScriptQuery: ''
                        }).then(function (res) {
                            try {
                                ...
                                if (payload && payload.success && payload.exists === true) {
                                    backendLog('galapasteam already present for this app; not inserting button');
                                    window.__galapasteamPresenceCheckInFlight = false;
                                    return; // do not insert
                                }
                                // Re-check in case another caller inserted during async
                                if (!document.querySelector('.galapasteam-button') && !window.__galapasteamButtonInserted) {
                                    // Insert after icon button (order: Restart → Icon → Add)
                                    const iconExisting = steamdbContainer.querySelector('.galapasteam-icon-button');
                                    const restartExisting = steamdbContainer.querySelector('.galapasteam-restart-button');
                                    if (iconExisting && iconExisting.before) {
                                        iconExisting.before(galapasteamButton);
                                    } else if (restartExisting && restartExisting.after) {
                                        restartExisting.after(galapasteamButton);
                                    } else if (referenceBtn && referenceBtn.after) {
                                        referenceBtn.after(galapasteamButton);
                                    } else {
                                        steamdbContainer.appendChild(galapasteamButton);
                                    }
                                    window.__galapasteamButtonInserted = true;
                                    backendLog('galapasteam button inserted');
                                }
                                window.__galapasteamPresenceCheckInFlight = false;
                            } catch (_) {
                                if (!document.querySelector('.galapasteam-button') && !window.__galapasteamButtonInserted) {
            ...
            try {
                ...
                if (!isNaN(appid)) {
                    const pillBtn = steamdbContainer.querySelector('.galapasteam-button');
                    if (pillBtn) {
                        // Skip if pills already built for this appid
                        var existingPills = pillBtn.querySelector('.galapasteam-pills-container');
                        if (!(existingPills && existingPills.dataset.appid === String(appid) && existingPills.dataset.content)) {
                            fetchGamesDatabase().then(function (db) {
                                const btn = steamdbContainer.querySelector('.galapasteam-button');
                                if (!btn) return;

                                let pillsContainer = btn.querySelector('.galapasteam-pills-container');

                                if (!pillsContainer) {
                                    pillsContainer = document.createElement('div');
                                    pillsContainer.className = 'galapasteam-pills-container';
                                ...
                                fetchFixes(appid).then(function (fixesData) {
                                    ...
                                    if (status !== 'untested') {
                                        ...
                                        pill.className = 'galapasteam-pill';
                                        if (status === 'playable') {
                                    ...
                                    if (btn) {
                                        ...
                                        if (span && span.textContent === 'Unplayable') {
                                            span.textContent = lt('Add via GalapaSteam');
                                        }
                                    }

                                    if (showDenuvoPill) {
                                        const pill = document.createElement('span');
                                        pill.className = 'galapasteam-pill orange';
            ...
            if (!logState.missingOnce) {
                backendLog('galapasteam: steamdbContainer not found on this page');
    ...
    function onFrontendReady() {
            ...
            fetchSettingsConfig(true).then(function (cfg) {
                ...
                try {
                    if (window.location.hostname === 'store.steampowered.com') {
                        if (localStorage.getItem('galapasteam millennium disclaimer accepted') !== '1') {
        ...
        setTimeout(function () {
            if (window.GamepadNav && window.GamepadNav.isConnected && window.GamepadNav.isConnected()) {
                backendLog('[galapasteam] Gamepad detected - Navigation enabled');

                // Only show visual hint in Big Picture mode
                if (window.__galapasteam_IS_BIG_PICTURE__) {
                    const hint = document.createElement('div');
                    hint.id = 'galapasteam-gamepad-hint';
                    ...
                    if (!document.querySelector('#galapasteam-gamepad-hint-styles')) {
                        const style = document.createElement('style');
                        style.id = 'galapasteam-gamepad-hint-styles';
                        style.textContent = '\
                            @keyframes fadeInOut {\
        ...
        try {
            if (typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                Millennium.callServerMethod('galapasteam', 'GetInitApisMessage', {
                    contentScriptQuery: ''
                }).then(function (res) {
                    try {
                        const payload = typeof res === 'string' ? JSON.parse(res) : res;
                        if (payload && payload.message) {
                            ...
                            if (isUpdateMsg) {
                                ...
                                showgalapasteamConfirm('galapasteam', msg, function () {
                ...
                try {
                    if (window.location.hostname === 'store.steampowered.com') {
                        if (!sessionStorage.getItem('galapasteamLoadedAppsGate')) {
                            sessionStorage.setItem('galapasteamLoadedAppsGate', '1');
                            Millennium.callServerMethod('galapasteam', 'ReadLoadedApps', {
                                contentScriptQuery: ''
                            }).then(function (res) {
                                try {
                                    const payload = typeof res === 'string' ? JSON.parse(res) : res;
    ...
    document.addEventListener('click', function (evt) {
        ...
        const anchor = evt.target.closest('.galapasteam-button');
        if (anchor) {
            evt.preventDefault();
            evt.stopPropagation(); // Stop propagation to avoid conflicts
            backendLog('galapasteam delegated click');
            try {
                const match = window.location.href.match(/https:\/\/store\.steampowered\.com\/app\/(\d+)/) || window.location.href.match(/https:\/\/steamcommunity\.com\/app\/(\d+)/);
                const appid = match ? parseInt(match[1], 10) : NaN;
                if (!isNaN(appid) && typeof Millennium !== 'undefined' && typeof Millennium.callServerMethod === 'function') {
                    if (runState.inProgress && runState.appid === appid) {
                        backendLog('galapasteam: operation already in progress for this appid');
                        return;
                    }

                    // Helper that continues with the normal add flow
                    const continueWithAdd = function () {
                        if (!document.querySelector('.galapasteam-overlay')) {
                            showTestPopup();
                        }
                        runState.inProgress = true;
                        runState.appid = appid;
                        Millennium.callServerMethod('galapasteam', 'StartAddViagalapasteam', {
                            appid,
                            contentScriptQuery: ''
                        });
                        startPolling(appid);
                    ...
                    fetch('https://store.steampowered.com/api/appdetails?appids=' + appid + '&filters=basic')
                            ...
                            return fetchGamesDatabase().then(function (db) {
                                try {
                                    ...
                                    if (gameData && gameData.playable === 0) {
                                        // warning modal
                                        showgalapasteamPlayableWarning('This game may not work, support for it wont be given in our discord', function () {
                                            continueWithAdd();
                                        }, function () { });
                                    } else {
                                        continueWithAdd();
                        ...
                        .catch(function (err) {
                            backendLog('galapasteam: DLC check failed: ' + err);
                            continueWithAdd();
                        });
                }
            } catch (_) { }
    ...
    function startPolling(appid) {
        ...
        const timer = setInterval(() => {
            ...
            try {
                Millennium.callServerMethod('galapasteam', 'GetAddViagalapasteamStatus', {
                    appid,
                    contentScriptQuery: ''
                }).then(function (res) {
                    try {
                        ...
                        const overlay = document.querySelector('.galapasteam-overlay');
                        const title = overlay ? overlay.querySelector('.galapasteam-title') : null;
                        const status = overlay ? overlay.querySelector('.galapasteam-status') : null;
                        const wrap = overlay ? overlay.querySelector('.galapasteam-progress-wrap') : null;
                        const progressInfo = overlay ? overlay.querySelector('.galapasteam-progress-info') : null;
                        const percent = overlay ? overlay.querySelector('.galapasteam-percent') : null;
                        const downloadSize = overlay ? overlay.querySelector('.galapasteam-download-size') : null;
                        const bar = overlay ? overlay.querySelector('.galapasteam-progress-bar') : null;

                        // Update individual API status in the list
                        if (overlay) {
                            const colors = getThemeColors();
                            const apiItems = overlay.querySelectorAll('.galapasteam-api-item');
                            ...
                            if ((st.status === 'downloading' || st.status === 'processing' || st.status === 'installing' || st.status === 'done') && st.currentApi && !successfulApi) {
                                ...
                                apiItems.forEach((item) => {
                                    ...
                                    const apiStatus = item.querySelector('.galapasteam-api-status');
                                    if (!apiStatus) return;
                        ...
                        if (st.status === 'checking' && st.currentApi && title) {
                            title.textContent = lt('galapasteam · {api}').replace('{api}', st.currentApi);
                        ...
                        if (st.status === 'downloading' || st.status === 'processing' || st.status === 'installing') {
                            ...
                            const cancelBtn = overlay ? overlay.querySelector('.galapasteam-cancel-btn') : null;
                            if (cancelBtn && st.status === 'downloading') cancelBtn.style.display = '';
                        }
                        if (st.status === 'done') {
                            // Update popup if visible
                            if (title) title.textContent = t('common.appName', 'galapasteam');
                            ...
                            const cancelBtn = overlay ? overlay.querySelector('.galapasteam-cancel-btn') : null;
                            if (cancelBtn) cancelBtn.style.display = 'none';
                            const hideBtn = overlay ? overlay.querySelector('.galapasteam-hide-btn') : null;
                            if (hideBtn) hideBtn.innerHTML = '<span>' + lt('Close') + '</span>';
                            ...
                            const btnEl = document.querySelector('.galapasteam-button');
    ...
    setTimeout(addgalapasteamButton, 1000);
    setTimeout(addgalapasteamButton, 3000);
    ...
    function showLoadedAppsPopup(apps) {
        // Avoid duplicates
        if (document.querySelector('.galapasteam-loadedapps-overlay')) return;
        ensureFontAwesome();
        ensuregalapasteamStyles();
        const overlay = document.createElement('div');
        ...
        overlay.className = 'galapasteam-loadedapps-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;animation:fadeIn 0.2s ease-out;';
        overlay.className = 'galapasteam-loadedapps-overlay';
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.75);backdrop-filter:blur(8px);z-index:99999;display:flex;align-items:center;justify-content:center;';
        const modal = document.createElement('div');
        ...
        title.textContent = lt('galapasteam · Added Games');
        const body = document.createElement('div');
        ...
        dismissBtn.className = 'galapasteam-btn';
        dismissBtn.innerHTML = '<span>' + lt('Dismiss') + '</span>';
        dismissBtn.href = '#';
        dismissBtn.onclick = function (e) {
            e.preventDefault();
            try {
                Millennium.callServerMethod('galapasteam', 'DismissLoadedApps', {
                    contentScriptQuery: ''
                });
            } catch (_) { }
            try {
                sessionStorage.setItem('galapasteamLoadedAppsShown', '1');
            } catch (_) { }
            overlay.remove();
        };
